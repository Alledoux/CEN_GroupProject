...
export default function Tasks({ onLogout }) {
    ...
    const addTask = async (e) => {
        ...
        try {
            ...
            setForm({ title: "", deadline: "", importance: "2", difficulty: "3", category: "Personal" });
        } catch {
    ...
    const getCalendarEvents = () =>
        tasks
            ...
            .map((task) => ({
                ...
                allDay: false,
                resource: task.category // Pass category to the event
            }));
    ...
    const eventStyleGetter = (event) => {
        ...
        const backgroundColor = categoryColors[event.resource] || '#6b7280'; // Default to gray
        const style = {
            backgroundColor,
            borderRadius: '5px',
            opacity: 0.8,
        ...
        <>
            ...
            <main className="tasks-wrapper">
                ...
                <div className="view-toggle">
                    ...
                    <button onClick={() => setViewMode("calendar")} className={viewMode === "calendar" ? "active" : ""}>Calendar</button>
                </div>

                {viewMode === "list" && (
                    <>
                        <form className="task-form" onSubmit={addTask}>
                            <input placeholder="Task title" value={form.title} onChange={handleForm("title")} />
                            <input type="datetime-local" value={form.deadline} onChange={handleForm("deadline")} />
                            ...
                            <input type="number" min="1" max="5" value={form.difficulty} onChange={handleForm("difficulty")} placeholder="Diff" />
                        ...
                        <ul className="task-grid">
                            {tasks.map((t) => {
                                ...
                                //  Add a class based on the category for styling
                                const categoryClass = `category-${t.category?.toLowerCase()}`;

                                return (
                                    <li key={t._id} className={`card ${overdue ? "overdue" : ""} ${categoryClass}`}>
                                        <input type="checkbox" checked={t.completion === 100} onChange={() => toggleComplete(t)} />
                                        <h2 className={t.completion === 100 ? "strike" : ""}>{t.title}</h2>
                                        <div className="meta">
                                            <span className={badge}>{pri}</span>
                                            {t.deadline && <span className="due">{dueTxt}</span>}
                                        </div>
                                        <button className="edit" onClick={() => setEditingTask(t)}>&#9998;</button>
                                        <button className="del" onClick={() => delTask(t._id)}>&times;</button>
                                    </li>
                                );
                            })}
                        </ul>
                ...
                {viewMode === "calendar" && (
                    <div className="calendar-container">
                        <Calendar
                            ...
                            views={allowedViews}
                            style={{ height: "70vh" }}
                            onSelectEvent={(event) => setEditingTask(tasks.find((t) => t._id === event.id))}
                            eventPropGetter={eventStyleGetter} // Apply styles to calendar events
                        />
                    </div>
                )}
            </main>
